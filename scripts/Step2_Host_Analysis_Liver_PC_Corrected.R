...
rawCounts1 <- read.delim("~/Desktop/salmon_project/Gene_Counts/featurecounts_X204SC21043349-Z02-F001_1_genome")
rawCounts2 <- read.delim("~/Desktop/salmon_project/Gene_Counts/featurecounts_X204SC21043349-Z02-F001_2_genome")
rawCounts3 <- read.delim("~/Desktop/salmon_project/Gene_Counts/featurecounts_X204SC21043349-Z02-F001_3_genome")
rawCounts4 <- read.delim("~/Desktop/salmon_project/Gene_Counts/featurecounts_X204SC21043349-Z02-F001_4_genome")

rawCounts_total <- join_all(list(rawCounts1, rawCounts2, rawCounts3, rawCounts4), by = 'Geneid', type = 'full')

mapping_file <- read.table("~/Desktop/salmon_project/SampleData.txt", header = T, sep = "\t", row.names = 1)


# removing the unnecessary information from the column names
colnames(rawCounts_total)<-gsub("X.home.projects.ku.cbd.data.HoloFish.IMPROVAFISH.ANALYSES_HostRNA.5_star_2pass.X204SC21043349.Z02.F001_1.","",colnames(rawCounts_total))
colnames(rawCounts_total)<-gsub("X.home.projects.ku.cbd.data.HoloFish.IMPROVAFISH.ANALYSES_HostRNA.5_star_2pass.X204SC21043349.Z02.F001_2.","",colnames(rawCounts_total))
colnames(rawCounts_total)<-gsub("X.home.projects.ku.cbd.data.HoloFish.IMPROVAFISH.ANALYSES_HostRNA.5_star_2pass.X204SC21043349.Z02.F001_3.","",colnames(rawCounts_total))
colnames(rawCounts_total)<-gsub("X.home.projects.ku.cbd.data.HoloFish.IMPROVAFISH.ANALYSES_HostRNA.5_star_2pass.X204SC21043349.Z02.F001_4.","",colnames(rawCounts_total))
colnames(rawCounts_total)<-gsub("_Aligned.sortedByCoord.out.bam","",colnames(rawCounts_total))
colnames(rawCounts_total)<-gsub("_Aligned.sortedByCoord.out.bam","",colnames(rawCounts_total))
colnames(rawCounts_total)<-gsub("\\..*","",colnames(rawCounts_total))


# preprocessing/filtering the dataframe by removing the raws with 0s
rawCounts_total <- data.frame(rawCounts_total[,-1], row.names = rawCounts_total[,1]) #making first column to row names
rawCounts_total <- subset(rawCounts_total, select=-c(Chr,Start, End)) #removing first few columns
#rawCounts_total <- rawCounts_total[rowSums(rawCounts_total[])>0,] #removing the raws with 0s
#library("dplyr")
#rawCounts_total <- tibble::rownames_to_column(rawCounts_total, "ensgene")

#Remove blanks
mapping_file <- mapping_file[!mapping_file$Sample_Type %in% c("BLK" ,"CL1", "CP1", "CP2", "CP3", "Gills", "Hind_Gut"), ]
rawCounts_total1 <- subset(rawCounts_total, select=-c(BLK ,CL1, CP1, CP2, CP3 ))

#Remove GILLS
rawCounts_total1 <- rawCounts_total1[ , !grepl(c("_Gih") , names(rawCounts_total1))]
rawCounts_total1 <- rawCounts_total1[ , !grepl(c("_HGh") , names(rawCounts_total1))]

mapping_file <- mapping_file[!mapping_file$Time %in% c("T0"), ]
rawCounts_total1 <- rawCounts_total1[ , !grepl(c("T0_ctr_") , names(rawCounts_total1))]
...
colnames(rawCounts_total1) <- gsub("_Lh0", "_",  colnames(rawCounts_total1))

all(colnames(rawCounts_total1) %in% rownames(mapping_file))
all(colnames(rawCounts_total1) == rownames(mapping_file))

reorder_idx <- match(rownames(mapping_file), colnames(rawCounts_total1))
rawCounts_total2 <- rawCounts_total1[ , reorder_idx]
all(colnames(rawCounts_total2) %in% rownames(mapping_file))
all(colnames(rawCounts_total2) == rownames(mapping_file))
...
modules.omics_X_Liver_PC_Corrected <- blockwiseModules(omics_data_host, 
                                    power = st, 
                                    networkType = "signed", 
                                    TOMType = "signed",
...
rownames(modules.omics_X_Liver_PC_Corrected$MEs) <- rownames(omics_data_host)
names(modules.omics_X_Liver_PC_Corrected$colors) <- colnames(omics_data_host)
names(modules.omics_X_Liver_PC_Corrected$unmergedColors) <- colnames(omics_data_host)

hubs <- chooseTopHubInEachModule(omics_data_host, modules.omics_X_Liver_PC_Corrected$colors, omitColors = 0)
hubs <- chooseTopHubInEachModule(omics_data_host, modules.omics_X_Liver_PC_Corrected$colors, power = st, omitColors = "0")

stage2results_X_Liver_PC_Corrected <- list(modules = modules.omics_X_Liver_PC_Corrected, 
                        hubs = hubs)


# Convert labels to colors for plotting
merged_colors <- labels2colors(stage2results_X_Liver_PC_Corrected$modules$colors)
n_modules <- unique(merged_colors) %>% length()

samples_good <- sum(stage2results_X_Liver_PC_Corrected$modules$goodSamples) == length(stage2results_X_Liver_PC_Corrected$modules$goodSamples)
genes_good <- sum(stage2results_X_Liver_PC_Corrected$modules$goodGenes) == length(stage2results_X_Liver_PC_Corrected$modules$goodGenes)

ME_good <- sum(stage2results_X_Liver_PC_Corrected$modules$MEsOK) == length(stage2results_X_Liver_PC_Corrected$modules$MEsOK)
table(stage2results_X_Liver_PC_Corrected$modules$colors) %>% 
  as.data.frame() %>% 
...
for(i in seq_along(stage2results_X_Liver_PC_Corrected$modules$dendrograms)){
  plotDendroAndColors(stage2results_X_Liver_PC_Corrected$modules$dendrograms[[i]], merged_colors[stage2results_X_Liver_PC_Corrected$modules$blockGenes[[i]]],
                      "Module colors",
                      ...
                      main = paste0("Cluster Dendrogram\n", 
                                    "for block ", 
                                    i,": ",
                                    length(stage2results_X_Liver_PC_Corrected$modules$blockGenes[[i]]),
                                    " genes"))
}


MEs <- stage2results_X_Liver_PC_Corrected$modules$MEs
...
modules <- 
  colnames(stage2results_X_Liver_PC_Corrected$modules$MEs)[colnames(stage2results_X_Liver_PC_Corrected$modules$MEs) %>% sub("ME", "", .) %>% as.numeric() %>% order()]

for(m in colnames(stage2results_X_Liver_PC_Corrected$modules$MEs)){
  h <- as.numeric(sub("ME","", m))
  data.frame(x = suppressWarnings(corr_within_module(omics_data_host = omics_data_host, modules = stage2results_X_Liver_PC_Corrected$modules, module_x = h))) %>% 
    ggplot() + 
...
#Hub genes
stage2results_X_Liver_PC_Corrected$hubs %>% 
  as.data.frame() %>% 
...
#https://alexslemonade.github.io/refinebio-examples/04-advanced-topics/network-analysis_rnaseq_01_wgcna.html
#modules.omics_X_Liver_PC_Corrected.backup <- modules.omics_X_Liver_PC_Corrected 
module_eigengenes <- modules.omics_X_Liver_PC_Corrected$MEs
...
gene_module_key <- tibble::enframe(modules.omics_X_Liver_PC_Corrected$colors, name = "gene", value = "module") %>%
...
moduleLabels <- modules.omics_X_Liver_PC_Corrected$colors
moduleColors <- labels2colors(modules.omics_X_Liver_PC_Corrected$colors)
table(moduleColors)
table(moduleLabels)

table(moduleColors, moduleLabels)

MEs <- modules.omics_X_Liver_PC_Corrected$MEs
geneTree <- modules.omics_X_Liver_PC_Corrected$dendrograms[[1]]
...
resGO.darkgreen <- enrichGO(gene = darkgreen.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
resGO.darkgrey <- enrichGO(gene = darkgrey.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
resGO.darkmagenta <- enrichGO(gene = darkmagenta.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
resGO.darkolivegreen <- enrichGO(gene = darkolivegreen.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
resGO.darkorange <- enrichGO(gene = darkorange.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
resGO.darkred <- enrichGO(gene = darkred.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
resGO.darkturquoise <- enrichGO(gene = darkturquoise.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
...
resGO.lightyellow <- enrichGO(gene = lightyellow.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
resGO.magenta <- enrichGO(gene = magenta.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
resGO.mediumpurple3 <- enrichGO(gene = mediumpurple3.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
...
resGO.orange <- enrichGO(gene = orange.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
resGO.orangered4 <- enrichGO(gene = orangered4.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
resGO.paleturquoise <- enrichGO(gene = paleturquoise.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
resGO.pink <- enrichGO(gene = pink.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
resGO.plum1 <- enrichGO(gene = plum1.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
...
resGO.royalblue <- enrichGO(gene = royalblue.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
resGO.saddlebrown <- enrichGO(gene = saddlebrown.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
resGO.salmon <- enrichGO(gene = salmon.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
resGO.sienna3 <- enrichGO(gene = sienna3.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
resGO.skyblue <- enrichGO(gene = skyblue.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
resGO.skyblue3 <- enrichGO(gene = skyblue3.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
resGO.steelblue <- enrichGO(gene = steelblue.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
...
resGO.violet <- enrichGO(gene = violet.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
resGO.white <- enrichGO(gene = white.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
resGO.yellow <- enrichGO(gene = yellow.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
resGO.yellowgreen <- enrichGO(gene = yellowgreen.df$V1, ont = "BP", universe = all.df$V1, OrgDb = SsalOrg, pvalueCutoff  = 0.01,qvalueCutoff  = 0.05)
...
a5 <- dotplot(resGO.darkgreen, showCategory = 10 )
dotplot(resGO.darkgrey, showCategory = 10 )
dotplot(resGO.darkmagenta, showCategory = 10 )
a8 <- dotplot(resGO.darkolivegreen, showCategory = 10 )
a9 <- dotplot(resGO.darkorange, showCategory = 10 )
a10 <- dotplot(resGO.darkred, showCategory = 10 )
a11 <- dotplot(resGO.darkturquoise, showCategory = 10 )
a12 <- dotplot(resGO.green, showCategory = 10 )
a13 <- dotplot(resGO.greenyellow, showCategory = 10 )
dotplot(resGO.grey60, showCategory = 10 )
a16 <- dotplot(resGO.lightcyan, showCategory = 10 )
a17 <- dotplot(resGO.lightgreen, showCategory = 10 )
a18 <- dotplot(resGO.lightyellow, showCategory = 10 )
a19 <- dotplot(resGO.magenta, showCategory = 10 )
dotplot(resGO.mediumpurple3, showCategory = 10 )
a21 <- dotplot(resGO.midnightblue, showCategory = 10 )
dotplot(resGO.orange, showCategory = 10 )
a23 <- dotplot(resGO.orangered4, showCategory = 10 )
dotplot(resGO.paleturquoise, showCategory = 10 )
a25 <- dotplot(resGO.pink, showCategory = 10 )
a26 <- dotplot(resGO.plum1, showCategory = 10 )
...
dotplot(resGO.steelblue, showCategory = 10 )
a36 <- dotplot(resGO.tan, showCategory = 10 )
...
a41 <- dotplot(resGO.yellowgreen, showCategory = 10 )

cowplot::plot_grid(a2,a3, a4, a5, a8, a9, a10, a11, a12, a13, a16, a17, ncol=4, labels = "auto")
cowplot::plot_grid(a18,a19, a21, a23, a25, a26, a27, a28, a36, a37, a40, a41, ncol=4, labels = "auto")

...
#Transcriptomics
#stage2results_X_Liver_PC_Corrected_backup <- stage2results_X_Liver_PC_Corrected
#stage2results_X_Liver_PC_Corrected <- stage2results_X_Liver_PC_Corrected_backup
################################################################################
stage2results_X_Liver_PC_Corrected


# Remove ME0 from analysis
idx <- which(colnames(stage2results_X_Liver_PC_Corrected$modules$MEs) == "ME0")
stage2results_X_Liver_PC_Corrected$modules$MEs <- stage2results_X_Liver_PC_Corrected$modules$MEs[,-idx]
filter.sig.mod <- 2
...
if(take_average){
  stage2results_X_Liver_PC_Corrected$modules$MEs %>% 
    rownames_to_column(var = "common") %>%
    mutate(common = sub("_[0-9]{1,2}$", "", common)) %>% 
    group_by(common) %>% 
    summarise_all(list(mean)) %>% 
    ungroup() -> stage2results_X_Liver_PC_Corrected_eigengenes
  
  stage2results_X_Liver_PC_Corrected_eigengenes <- as.data.frame(stage2results_X_Liver_PC_Corrected_eigengenes)
  
} else{
  stage2results_X_Liver_PC_Corrected_eigengenes <- stage2results_X_Liver_PC_Corrected$modules$MEs
}

rownames(stage2results_X_Liver_PC_Corrected_eigengenes) <- stage2results_X_Liver_PC_Corrected_eigengenes$common
stage2results_X_Liver_PC_Corrected_eigengenes$common <- NULL

# Create a dendrogram of the transcriptomics eigengenes to organise the final plots.
X_ME_dendro <- hclust(as.dist(1 - WGCNA::bicor(stage2results_X_Liver_PC_Corrected_eigengenes, maxPOutliers = 0.05)), method = "ward.D2")
...
annotation_col <- 
  rownames(stage2results_X_Liver_PC_Corrected_eigengenes) %>% 
  as.data.frame() %>% 
  dplyr::rename(common = ".") %>% 
...
stage2results_X_Liver_PC_Corrected_eigengenes_to_plot <- 
  dplyr::inner_join(annotation_col %>% 
                      rownames_to_column(var = "common"), 
                    stage2results_X_Liver_PC_Corrected_eigengenes %>% 
                      rownames_to_column(var = "common"), 
                    by = "common") %>%
  ...
  t()
library(WGCNA)
moduleLabels = stage2results_X_Liver_PC_Corrected$modules$colors
moduleColors = labels2colors(stage2results_X_Liver_PC_Corrected$modules$colors)
table(moduleLabels)
table(moduleColors)
...
pheatmap::pheatmap(stage2results_X_Liver_PC_Corrected_eigengenes_to_plot, 
                   cluster_cols = F,
                   ...
                   labels_row = paste0(prefix_genes, rownames(stage2results_X_Liver_PC_Corrected_eigengenes_to_plot)),
...
if(take_average){
  stage2results_Y_PC_corrected$modules$MEs %>% 
    ...
    group_by(common) %>% 
    summarise_all(list(mean)) %>% 
    ungroup() -> stage2results_Y_PC_corrected_eigengenes
  
  stage2results_Y_PC_corrected_eigengenes <- as.data.frame(stage2results_Y_PC_corrected_eigengenes)
} else{
  stage2results_Y_PC_corrected_eigengenes <- stage2results_Y_PC_corrected$modules$MEs
}

rownames(stage2results_Y_PC_corrected_eigengenes) <- stage2results_Y_PC_corrected_eigengenes$common
stage2results_Y_PC_corrected_eigengenes$common <- NULL

dim(stage2results_Y_PC_corrected_eigengenes); dim(stage2results_X_Liver_PC_Corrected_eigengenes)
...
same_order <- all(rownames(stage2results_Y_PC_corrected_eigengenes) == rownames(stage2results_X_Liver_PC_Corrected_eigengenes))
if(!same_order){
  stage2results_Y_PC_corrected_eigengenes <- stage2results_Y_PC_corrected_eigengenes[order(rownames(stage2results_Y_PC_corrected_eigengenes)),]
  stage2results_X_Liver_PC_Corrected_eigengenes <- stage2results_X_Liver_PC_Corrected_eigengenes[order(rownames(stage2results_X_Liver_PC_Corrected_eigengenes)),]
  same_order <- all(rownames(stage2results_Y_PC_corrected_eigengenes) == rownames(stage2results_X_Liver_PC_Corrected_eigengenes))
  if(!same_order){
...
#####
p.value_matr <- corr.value_matr <- matrix(ncol = ncol(stage2results_Y_PC_corrected_eigengenes), 
                                          nrow = ncol(stage2results_X_Liver_PC_Corrected_eigengenes), 
                                          dimnames = list(colnames(stage2results_X_Liver_PC_Corrected_eigengenes), 
                                                          colnames(stage2results_Y_PC_corrected_eigengenes)))


for(i in 1:ncol(stage2results_X_Liver_PC_Corrected_eigengenes)){
  for(j in 1:ncol(stage2results_Y_PC_corrected_eigengenes)){
    cor.res <- cor.test(stage2results_X_Liver_PC_Corrected_eigengenes[,i], stage2results_Y_PC_corrected_eigengenes[,j])
...
dimnames(p.value_matr.adjust) <- list(colnames(stage2results_X_Liver_PC_Corrected_eigengenes), colnames(stage2results_Y_PC_corrected_eigengenes))
...
# Check that the samples are in the same order. 
# If they are not in order, change their order to match; If they do not match one-to-one, call an error.
same_order <- all(rownames(Z_traits) == rownames(stage2results_X_Liver_PC_Corrected_eigengenes))
if(!same_order){
  stage2results_Y_PC_corrected_eigengenes <- stage2results_Y_PC_corrected_eigengenes[order(rownames(stage2results_Y_PC_corrected_eigengenes)),]
  stage2results_X_Liver_PC_Corrected_eigengenes <- stage2results_X_Liver_PC_Corrected_eigengenes[order(rownames(stage2results_X_Liver_PC_Corrected_eigengenes)),]
  same_order <- all(rownames(stage2results_Y_PC_corrected_eigengenes) == rownames(stage2results_X_Liver_PC_Corrected_eigengenes))
...
for(i in 1:ncol(stage2results_X_Liver_PC_Corrected_eigengenes)){
  for(j in 1:ncol(Z_traits)){
    ...
    if(colnames(Z_traits)[j] == "Day"){
      cor.res <- cor.test(stage2results_X_Liver_PC_Corrected_eigengenes[,i], Z_traits[,j], method = "spearman", exact = FALSE)
      p.value_matr[i, j] <- cor.res$p.value
      corr.value_matr[i, j] <- cor.res$estimate
    } else{
      cor.res <- cor.test(stage2results_X_Liver_PC_Corrected_eigengenes[,i], Z_traits[,j])
      p.value_matr[i, j] <- cor.res$p.value
      corr.value_matr[i, j] <- cor.res$estimate
    }
  }
...
p.value_matr.adjust <- p.adjust(p.value_matr, method = "fdr")
dim(p.value_matr.adjust) <- dim(p.value_matr)
dimnames(p.value_matr.adjust) <- list(colnames(stage2results_X_Liver_PC_Corrected_eigengenes), colnames(Z_traits))
...
cowplot::plot_grid(Z_corr_X_plot$gtable,
                   ...
                   rel_widths = c(dim(Z_traits)[2]/3, 
                                  ifelse(take_average,dim(stage2results_X_Liver_PC_Corrected_eigengenes)[1]/2,dim(stage2results_X_Liver_PC_Corrected_eigengenes)[1]/6),
                                  dim(stage2results_Y_PC_corrected_eigengenes)[2]/2.2),
...
#                    ncol = 3, 
#   align = 'h',
#   rel_widths = c(0.9, 3.9, 1.9) #1600 (Width) * 1100 (Height)
...
moduleLabels1 = stage2results_X_Liver_PC_Corrected$modules$colors
moduleColors1 = labels2colors(stage2results_X_Liver_PC_Corrected$modules$colors)
MEs = stage2results_X_Liver_PC_Corrected$MEs;
...
table(rownames(MEs) %in% sample_info1$common)
...
moduleTraitCor <- moduleTraitCor[order(match(rownames(moduleTraitCor), c("ME25",	"ME22",	"ME9",	"ME34",	"ME33",	"ME24",	"ME43",	"ME52",	"ME12",	"ME40",	"ME44",	"ME36",	"ME30",	"ME19",	"ME32",	"ME35",	"ME29",	"ME42",	"ME46",	"ME31",	"ME13",	"ME41",	"ME39",	"ME49",	"ME20",	"ME37",	"ME21",	"ME6",	"ME47",	"ME15",	"ME45",	"ME2",	"ME1",	"ME38",	"ME28",	"ME50",	"ME23",	"ME16",	"ME4",	"ME18",	"ME8",	"ME17",	"ME10",	"ME26",	"ME11",	"ME48",	"ME7",	"ME5",	"ME14",	"ME51",	"ME27",	"ME3",	"ME53"))), ]
...
cowplot::plot_grid(MT_plot$gtable,
                   ...
                   ncol = 4,
                   align = 'h',
                   rel_widths = c(1.2, 1.5, 3.5, 0.9) # PNG 1500 (Width) * 900 (Height)  # pdf 16 * 10 portrait

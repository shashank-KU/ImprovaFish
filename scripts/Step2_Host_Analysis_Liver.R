...
omics_data_host_Liver <- rawCounts_total2
...
omics_data_host_Liver <- omics_data_host_Liver1
all(colnames(omics_data_host_Liver) == sample_info$common)
reorder_idx <- match(sample_info$common, colnames(omics_data_host_Liver))
omics_data_host_Liver <- omics_data_host_Liver[ , reorder_idx]
all(colnames(omics_data_host_Liver) %in% sample_info$common)
all(colnames(omics_data_host_Liver) == sample_info$common)
...
omics_data_host_Liver<- normalized_counts 
rm(normalized_counts)

omics_data_host_Liver <- omics_data_host_Liver %>% t()

dim(omics_data_host_Liver) %>% paste(c("genes", "Samples"))
...
if(pcCorrection){
  ...
  if(estimate_n.pc){
    ...
    print(t(omics_data_host_Liver) %>% dim() %>% paste(c("Samples", "genes"))) 
    mod=matrix(1,nrow=nrow(t(omics_data_host_Liver)),ncol=1)
    colnames(mod)="Intercept"
    
    print(omics_data_host_Liver %>% dim() %>% paste(c("genes", "Samples"))) 
    n.pc = num.sv(dat = omics_data_host_Liver, mod = mod,  method="be")
  } else{
    n.pc = number_of_pcs_to_remove
  }
  
  print(omics_data_host_Liver %>% dim() %>% paste(c("genes", "Samples")))
  omics_data_host_Liver <- sva_network(omics_data_host_Liver, n.pc = n.pc)
}
...
omics_data_host_Liver <- omics_data_host_Liver %>% t()
dim(omics_data_host_Liver) %>% paste(c("Samples", "genes"))

powers <- c(c(1:10), seq(from = 12, to=20, by=2))

library(WGCNA)
sft <- pickSoftThreshold(omics_data_host_Liver, 
                         powerVector = powers, 
                         verbose = set_verbose, 
                         networkType = "signed",
...
modules.omics_X_Liver <- blockwiseModules(omics_data_host_Liver, 
                                    power = st, 
                                    networkType = "signed", 
                                    TOMType = "signed",
...
rownames(modules.omics_X_Liver$MEs) <- rownames(omics_data_host_Liver)
names(modules.omics_X_Liver$colors) <- colnames(omics_data_host_Liver)
names(modules.omics_X_Liver$unmergedColors) <- colnames(omics_data_host_Liver)

hubs <- chooseTopHubInEachModule(omics_data_host_Liver, modules.omics_X_Liver$colors, omitColors = 0)
hubs <- chooseTopHubInEachModule(omics_data_host_Liver, modules.omics_X_Liver$colors, power = st, omitColors = "0")
...
all(rownames(omics_data_host_Liver) == rownames(MEs))
dim(omics_data_host_Liver) %>% paste0(c(" samples", " genes"))
kME <- bicor(omics_data_host_Liver, MEs, maxPOutliers = 0.05)
dim(kME) %>% paste0(c(" genes", " modules"))



# Corr within modules
corr_within_module <- function(omics_data_host_Liver, modules, module_x = 1){
  idx.omics_data_host_Liver <- which(modules$colors == module_x)
  idx.me <- which(colnames(modules$MEs) == paste0("ME",module_x))
  kME_x <- bicor(omics_data_host_Liver[,idx.omics_data_host_Liver], modules$MEs[,idx.me], maxPOutliers = 0.05)
  kME_x
}
...
for(m in colnames(stage2results_X_Liver$modules$MEs)){
  ...
  data.frame(x = suppressWarnings(corr_within_module(omics_data_host_Liver = omics_data_host_Liver, modules = stage2results_X_Liver$modules, module_x = h))) %>% 
    ggplot() + 
...
part_3 # 1300 * 1300 # for PDF 15 *15
...
probes <- colnames(omics_data_host_Liver) 

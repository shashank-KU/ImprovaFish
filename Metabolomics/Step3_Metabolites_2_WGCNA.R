...
for (i in c("T2", "T3")){
  for (j in c("MC1","MC2", "MN3")){
    ...
    lef_out<-run_lefse(phy,group = "New_Diet", norm = "CPM", 
                       kw_cutoff = 0.05, lda_cutoff = 2, taxa_rank = "none")
    assign(paste0("RP_metabolomics", i, j), plot_abundance(lef_out, group = "New_Diet"))
...
plot_grid(RP_metabolomicsT2MC1, RP_metabolomicsT2MC2, RP_metabolomicsT2MN3, 
          RP_metabolomicsT3MC1, RP_metabolomicsT3MC2, RP_metabolomicsT3MN3, 
          labels = "AUTO", ncol = 1, align = "hv", rel_heights = c(0.3,0.3,1.2,1.2,2.8,2.8))
# portrait 25 *10
